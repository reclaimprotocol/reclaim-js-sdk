...
import { TeeVerificationError } from '../utils/errors';
...
describe('verifyProof', () => {
    ...
    it('returns TeeVerificationError when TEE data is missing', async () => {
        const proof = cloneProof();
        delete (proof as any).teeAttestation;
        // Re-sign would be needed to test TEE-missing path via verifyProof,
        // so test the TEE check directly instead
        const context = JSON.parse(proof.claimData.context);
        delete context.attestationNonce;
        proof.claimData.context = JSON.stringify(context);
        // Since modifying context invalidates signature, this fails at signature check first
        const result = await verifyProof(proof, { dangerouslyDisableContentValidation: true }, true);
        expect(result.isVerified).toBe(false);
        expect(result.error).toBeInstanceOf(Error);
        expect(result.data).toEqual([]);
