...
export interface ProviderHashRequirementSpec {
    /** List of HTTP requests that must be present in the proof */
    requiredRequests: RequestSpec[] | undefined;
    /**
     * List of optional extra HTTP requests permitted in the proof.
     * Such proofs are generated by custom injection scripts from providers which use `window.Reclaim.requestClaim` when running as User Script in user's browser. 
     */
    allowedExtraRequests: RequestSpec[] | undefined;
    /** If true, the validation allows additional proofs beyond the requiredRequests ones only when allowedExtraRequests is empty */
...
export type ProviderHashRequirementsConfig = {
    ...
    /** Array of explicitly allowed extra computed hashes */
    allowedExtraHashes: string[];
    /** Determines if unknown arbitrary extra proofs are allowed to pass validation after proofs from requiredRequests are validated when allowedExtraRequests is empty */
    allowArbitraryExtras: boolean;
}
